...
(async()=>{
 ...
 const pl=page.frames().find(f=>f.url().endsWith('/planning.html'));
 ...
 await page.reload();await page.waitForFunction(()=>document.querySelector('#status').textContent.includes('Werkruimte gereed'));await page.locator('#primaryNav [data-route=planning]').click();const restored=page.frames().find(f=>f.url().endsWith('/planning.html'));await restored.locator('#iplCanvas').waitFor({state:'visible'});assert((await restored.locator('#iplCanvas').boundingBox()).width>100);
