...
import {createServiceExpertReview,normalizeServiceExpertReview,expertReviewCompletion,validateServiceExpertReview,upsertServiceExpertReview} from './core/service-expert-input.js';
...
function expertField(label,name,value,help='',type='textarea'){
 const control=type==='input'?`<input name="${name}" value="${esc(value)}">`:type==='date'?`<input name="${name}" type="date" value="${esc(value)}">`:`<textarea name="${name}" rows="3">${esc(value)}</textarea>`;
...
function renderExpertInput(){
 ...
 const service=expertService(),stored=expertStored(service.id),cached=expertDrafts.get(service.id),review=expertDraft&&expertDraft.dienstId===service.id?expertDraft:cached||normalizeServiceExpertReview(stored||createServiceExpertReview(service),service),progress=expertReviewCompletion(review);
 expertDraft=review;
