...
| DRIP totaal (JSON) en uit map | `tests/drip-totaal.test.js`: `sbDripEventsUitTekst()` parseert tab-gescheiden CDMS-logregels (status URGENT/OK), `sbBouwDripBundel()` sluit episodes en houdt een blijvend alarm open, `sbPadInfoDrip()`/`sbFilterDripBestanden()` accepteren alleen cdms/<vc>/log en filteren op regio/periode, `sgVolgendeCtxDrip()` snoeit de maptraversal, `dripDatasetNaarBron()` zet `datasets.drip` om naar incidenten en een open episode wordt door `drip-open-from-history.js` als openstaand herkend, en `sbCombineerMetDripBasis()` vervangt alleen de gekozen regio. `tests/dvm-source-manager.test.js` legt vast dat de losse DRIP-historie-upload is vervangen door de bronnen DRIP totaal en DRIP uit map |
